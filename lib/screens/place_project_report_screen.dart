// lib/screens/place_project_report_screen.dart
...
/// เมนูรายงาน 9 > แบบขออนุญาตใช้สถานที่และบริการ (โครงการ)
///
/// ต่างจากรายงานอื่นตรงที่พิมพ์ทีละใบจอง ปุ่มพิมพ์จึงอยู่ท้ายแถว
/// ไม่ใช่ปุ่มเดียวสำหรับผลค้นหาทั้งหมด
class PlaceProjectReportScreen extends StatefulWidget {
  final AuthProvider authProvider;
  final ApiService apiService;
  const PlaceProjectReportScreen({
    super.key,
  ...
  @override
  State<PlaceProjectReportScreen> createState() =>
      _PlaceProjectReportScreenState();
}

class _PlaceProjectReportScreenState
    extends State<PlaceProjectReportScreen> {
  // Pagination
  int _currentPage = 0;
  int _pageSize = 5;
  int _totalItems = 0;
  ...
  /// ถามรูปแบบไฟล์แล้วออกแบบขออนุญาต (โครงการ) ของใบจองนั้นใบเดียว
  Future<void> _printOne(int bookId) async {
    final format = await _askFormat();
    if (format == null || !mounted) return;

    setState(() => _printing = true);
    try {
      final bytes = await widget.apiService.downloadPlaceProjectReport(
        bookId: bookId,
        format: format,
      ...
      final anchor = html.document.createElement('a') as html.AnchorElement
        ..href = url
        ..style.display = 'none'
        ..download = 'แบบขออนุญาตโครงการ_$bookId.${isPdf ? 'pdf' : 'xlsx'}';
