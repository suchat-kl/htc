// lib/screens/activity_room_search_screen.dart
...
/// เมนูห้องพัก/ห้องกิจกรรม > ค้นหาห้องกิจกรรม
///
/// ส่วนบนเป็นเงื่อนไขค้นหา 7 ช่อง กรอกบางช่องหรือไม่กรอกเลยก็ได้
...
class ActivityRoomSearchScreen extends StatefulWidget {
  final ApiService apiService;
  final AuthProvider authProvider;

  const ActivityRoomSearchScreen({
    super.key,
  ...
  @override
  State<ActivityRoomSearchScreen> createState() =>
      _ActivityRoomSearchScreenState();
}

class _ActivityRoomSearchScreenState extends State<ActivityRoomSearchScreen> {
  /// m_roomtype.type ของห้องกิจกรรม
  static const String _roomType = 'C';
  ...
  Future<void> _search({int page = 0}) async {
    ...
    try {
      final res = await widget.apiService.searchRooms(
        type: _roomType,
      ...
      if (AppLogger.on) AppLogger.d('Error searching rooms: $e');
      if (!mounted) return;
      setState(() {
      ...
      children: [
        Wrap(
          ...
          children: [
            ...
            _textField('ผู้จอง', _contractNameCtrl, 240),
