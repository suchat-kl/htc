// lib/screens/lodging_usage_report_screen.dart
...
/// เมนูรายงาน 2 > รายงานการใช้ห้องพัก
///
/// โครงหน้าจอเดียวกับรายงานการใช้ห้องกิจกรรม ต่างกันที่ปุ่มพิมพ์ออกแบบรายงาน
/// การใช้ห้องพัก ซึ่งแยกบรรทัดตามกลุ่มสถานะห้อง Check in / Check out
class LodgingUsageReportScreen extends StatefulWidget {
  final AuthProvider authProvider;
  final ApiService apiService;
  const LodgingUsageReportScreen({
    super.key,
  ...
  @override
  State<LodgingUsageReportScreen> createState() =>
      _LodgingUsageReportScreenState();
}

class _LodgingUsageReportScreenState
    extends State<LodgingUsageReportScreen> {
  // Pagination
  int _currentPage = 0;
  int _pageSize = 5;
  int _totalItems = 0;
  ...
  /// ถามรูปแบบไฟล์แล้วออกแบบรายงานการใช้ห้องพักตามเงื่อนไขที่ค้นหาอยู่
  ///
  ...
  Future<void> _printReport() async {
    ...
    try {
      final bytes = await widget.apiService.downloadLodgingUsageReport(
        format: format,
        bookID: _bookIDCtrl.text.isNotEmpty
            ? int.tryParse(_bookIDCtrl.text)
            : null,
      ...
      final anchor = html.document.createElement('a') as html.AnchorElement
        ..href = url
        ..style.display = 'none'
        ..download = 'รายงานการใช้ห้องพัก.${isPdf ? 'pdf' : 'xlsx'}';
  ...
  Future<String?> _askFormat() {
    return showDialog<String>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('ต้องการออกรายงานการใช้ห้องพักเป็นไฟล์แบบใด'),
        actions: [
          TextButton(
