// lib/screens/place_request_report_screen.dart
...
/// เมนูรายงาน 8 > แบบขออนุญาตใช้สถานที่และบริการ
///
/// โครงหน้าจอเดียวกับรายงานการใช้ห้องพัก เพิ่มคอลัมน์วันที่จอง
/// ปุ่มพิมพ์ออกแบบฟอร์มขออนุญาตของใบจองที่อยู่ในผลค้นหา
class PlaceRequestReportScreen extends StatefulWidget {
  final AuthProvider authProvider;
  final ApiService apiService;
  const PlaceRequestReportScreen({
    super.key,
  ...
  @override
  State<PlaceRequestReportScreen> createState() =>
      _PlaceRequestReportScreenState();
}

class _PlaceRequestReportScreenState
    extends State<PlaceRequestReportScreen> {
  // Pagination
  int _currentPage = 0;
  int _pageSize = 5;
  int _totalItems = 0;
                    ...
                    child: Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          _buildCell(
                            item.bookdate != null
                                ? Util.formatThaiDateStr(item.bookdate)
                                : '-',
                            flex: 2,
                          ),
                          _buildCell(
  ...
  /// ถามรูปแบบไฟล์แล้วออกแบบขออนุญาตใช้สถานที่และบริการตามเงื่อนไขที่ค้นหาอยู่
  ///
  ...
  Future<void> _printReport() async {
    ...
    try {
      final bytes = await widget.apiService.downloadPlaceRequestReport(
        format: format,
        bookID: _bookIDCtrl.text.isNotEmpty
            ? int.tryParse(_bookIDCtrl.text)
            : null,
      ...
      final anchor = html.document.createElement('a') as html.AnchorElement
        ..href = url
        ..style.display = 'none'
        ..download = 'แบบขออนุญาตใช้สถานที่และบริการ.${isPdf ? 'pdf' : 'xlsx'}';
  ...
  Future<String?> _askFormat() {
    return showDialog<String>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('ต้องการออกแบบขออนุญาตใช้สถานที่และบริการเป็นไฟล์แบบใด'),
        actions: [
          TextButton(
