...
class StatusCheckScreen extends StatefulWidget {
  ...
  const StatusCheckScreen({super.key, required this.apiService});
...
class _StatusCheckScreenState extends State<StatusCheckScreen> {
  ...
  // Data
  List<StatusCheck> _statusChecks = [];
  bool _isLoading = false;
  String? _error;

  // Search — ค้นหาแยกช่อง รหัส และ ชื่อสถานะ
  ...
  Future<void> _loadData() async {
    ...
    try {
      final response = await widget.apiService.getStatusChecks(
        page: _currentPage,
        size: _pageSize,
      ...
      if (mounted) {
        setState(() {
          _statusChecks = (response['statuschecks'] as List)
              .map((j) => StatusCheck.fromJson(j))
              .toList();
  ...
  Future<void> _saveStatusCheck() async {
    if (!_formKey.currentState!.validate()) return;

    try {
      final statusCheck = StatusCheck(
        status: _editingItem?.status,
        name: _nameController.text.trim(),
      );

      if (_editingItem == null) {
        // Create
        await widget.apiService.createStatusCheck(statusCheck);
        if (mounted) {
          context.showSuccessSnackBar('เพิ่มสถานะห้องพักสำเร็จ');
        }
      } else {
        // Update
        await widget.apiService.updateStatusCheck(
          _editingItem!.status!,
          statusCheck,
        );
        if (mounted) {
          context.showSuccessSnackBar('อัปเดตสถานะห้องพักสำเร็จ');
        }
      }
  ...
  Future<void> _deleteStatusCheck(StatusCheck item) async {
    final confirmed = await AppDialog.showConfirm(
      context,
      'ต้องการลบสถานะ "${item.name}" นี้?',
      confirmText: 'ลบ',
      cancelText: 'ยกเลิก',
    );
    ...
    if (confirmed == true) {
      try {
        await widget.apiService.deleteStatusCheck(item.status!);
        if (mounted) {
          context.showSuccessSnackBar('ลบสถานะห้องพักสำเร็จ');
          await _loadData();
        }
      } catch (e) {
        if (mounted) {
  ...
  void _showAddEditDialog({StatusCheck? item}) {
    _editingItem = item;
        ...
        return AlertDialog(
          ...
          title: Row(
            children: [
              ...
              Text(
                item == null ? 'เพิ่มสถานะห้องพัก' : 'แก้ไขสถานะห้องพัก',
  ...
  Future<void> _runSearch() async {
    ...
    setState(() {
      _searchStatus = int.tryParse(_codeSearchCtrl.text.trim());
      _searchName = _nameSearchCtrl.text.trim();
      _currentPage = 0;
    });
    _loadData();
  }
                      ...
                      ? Center(
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 16),
                              Text(
                                (_searchStatus == null && _searchName.isEmpty)
