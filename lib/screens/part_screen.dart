...
class _PartScreenState extends State<PartScreen> {
  ...
  static const double _wUnit = 130;
  ...
  void _rebuildRows() {
    ...
    _rows
      ..clear()
      ..addAll(_parts.map(_EditRow.from));
  }
  ...
  Future<void> _saveRows() async {
    ...
    try {
      for (final id in _deletedIds) {
        await widget.apiService.deletePart(id);
      }
      _deletedIds.clear();
      ...
      for (final r in _rows) {
        final item = r.toPart();
        if (r.isNew) {
          await widget.apiService.createPart(item);
        } else if (r.changed) {
          await widget.apiService.updatePart(r.originalId!, item);
        }
      }
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _headerCell('จำนวน', _wStock, align: TextAlign.right),
          _headerCell('หน่วย', _wUnit),
  ...
  ///
  /// รหัส (partID) เป็นคีย์ที่ฐานข้อมูลสร้างเอง จึงไม่แสดงและแก้ไม่ได้
  /// ส่วน "จำนวน" ระบบคำนวณจากการเบิกจ่าย จึงแสดงอย่างเดียวเหมือนหน้าต่างเดิม
        ...
        children: [
          ...
          _cell(
            ...
            DropdownButtonFormField<String>(
              ...
              initialValue: _codeOrNull(row.type, _types),
              isExpanded: true,
          ...
          _cell(
            ...
            DropdownButtonFormField<String>(
              ...
              initialValue: _codeOrNull(row.status, _statuses),
              isExpanded: true,
              style: const TextStyle(fontSize: 14, color: Colors.black87),
              decoration: _rowInput(),
              items: _statuses
  ...
  String? _codeOrNull(String value, List<Map<String, String>> codes) {
    return codes.any((c) => c['code'] == value) ? value : null;
  }
...
/// หนึ่งแถวที่แก้ไขได้ในตารางวัสดุซ่อมบำรุง
class _EditRow {
  ...
  /// จำนวนคงเหลือ ระบบคำนวณให้ แก้ในตารางไม่ได้ (เหมือนหน้าต่างเพิ่ม/แก้ไข)
  ...
  factory _EditRow.from(Part p) => _EditRow(
    originalId: p.partID,
    originalSequence: '${p.sequence}',
    originalName: p.name ?? '',
    originalUnit: p.unit ?? '',
    originalType: p.type,
    originalStatus: p.status,
    stockLevel: p.stockLevel,
    sequenceCtrl: TextEditingController(text: '${p.sequence}'),
  ...
  bool get changed =>
      ...
      (sequenceCtrl.text.trim() != originalSequence.trim() ||
          ...
          unitCtrl.text.trim() != originalUnit.trim() ||
          type != originalType ||
